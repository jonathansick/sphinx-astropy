...
def generate_example_page(title, tags, example_id, filepath, h1header,
    ...
    h1header : str
        ...
        example, ``'#'``.
    src_docname : str
        The absolute docname of the example's source page.
    """
    if tags:
        sorted_tags = sorted(tags)
        tag_links = []
        for tagname in sorted_tags:
            tag_link = ':doc:`{name} <{doc}>`'.format(
                name=tagname, doc=taginfo[tagname]['abs_docname'])
            tag_links.append(tag_link)
        tag_text = 'Tagged: {tags}'.format(tags=', '.join(tag_links))
    else:
        tag_text = ''
